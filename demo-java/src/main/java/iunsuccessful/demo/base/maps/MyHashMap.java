...
public class MyHashMap<K,V> extends MyAbstractMap<K,V>
    ...
    static class Node<K,V> implements Map.Entry<K,V> {
        ...
        public final String toString() {
            String str = key + "=" + value + " hash: " + hash;
            if (this.next != null) {
                str += " Next: " + this.next.hash;
            }
            return str;
        }
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        System.out.printf("%d %s %d\n", n, Integer.toBinaryString(n-1), (n - 1) & hash);
...
//            return new MyKeySpliterator<K, V>(this, 0, -1, 0, 0);
            return null;
        }
...
//            SharedSecrets.getJavaObjectInputStreamAccess().checkArray(s, Map.Entry[].class, cap);
    ...
    static final class MyKeySpliterator<K,V>
            extends HashMapSpliterator<K,V>
            implements Spliterator<K> {
        MyKeySpliterator(MyHashMap<K,V> m, int origin, int fence, int est,
                         int expectedModCount) {
            super(m, origin, fence, est, expectedModCount);
        }

        public MyKeySpliterator<K,V> trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || current != null) ? null :
                    new MyKeySpliterator<>(map, lo, index = mid, est >>>= 1,
                            expectedModCount);
        }
